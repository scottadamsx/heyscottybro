...
export default function ArcadePage() {
  ...
  const Tile = ({ g }) => (
    <button type="button" className="arcade-tile" onClick={() => playGame(g)}>
      ...
      {best[g.id] > 0 && <span className="arcade-tile-best">Best <strong>{best[g.id]}</strong></span>}
    </button>
  );
    ...
    <div className="module-page arcade-page">
      <div className="module-header">
        ...
        <div className="header-actions">
          ...
          {import.meta.env.DEV && (
            <button type="button" className="btn btn-sm btn-secondary-sm" onClick={() => { earnBreak(1, "dev"); setTokens(breakTokens()); }} title="Dev: grant a break token">+1 token</button>
          )}
        </div>
      </div>
      ...
      <section className="arcade-section">
        <div className="arcade-section-head">
          <h2 className="arcade-sh">Quick breaks</h2>
          <span className="arcade-sh-meta">1–3 minutes</span>
        </div>
        <div className="arcade-grid">
          {quick.map((g) => <Tile key={g.id} g={g} />)}
          <button type="button" className="arcade-tile arcade-tile-read" onClick={() => playOpp(articleOpp)}>
            <span className="arcade-tile-title"><i className="fa-solid fa-book-open" aria-hidden="true" /> Reading break</span>
          ...
          {PULSES.map((p) => (
            <button key={p.id} type="button" className="arcade-tile arcade-tile-pulse" onClick={() => playPulse(p)}>
              <span className="arcade-tile-title"><i className="fa-solid fa-bolt" aria-hidden="true" /> {p.label}</span>
      ...
      <section className="arcade-section">
        <div className="arcade-section-head">
          <h2 className="arcade-sh">Check-ins</h2>
          <span className="arcade-sh-meta">Kiwi's quick surveys</span>
        </div>
          ...
          {Object.entries(surveysByArea).map(([area, opps]) => {
            ...
            return (
              <div key={area} className="arcade-area">
                <h3 className="arcade-area-h"><i className={`fa-solid ${meta.icon}`} aria-hidden="true" /> {meta.label} <span className="db-count arcade-area-n">{opps.length}</span></h3>
                <div className="arcade-chiprow">
                  {opps.map((o) => (
                    <button key={o.id} type="button" className="chip arcade-chip" title={o.description} onClick={() => playOpp(o)}>
      ...
      <section className="arcade-section">
        <div className="arcade-section-head">
          <h2 className="arcade-sh">The full cabinet</h2>
          <span className="db-count">{rest.length}</span>
          <input className="arcade-search" placeholder="Search games…" aria-label="Search games" value={search} onChange={(e) => setSearch(e.target.value)} />
        </div>
        <div className="arcade-grid">
      ...
      {labOpps.length > 0 && (
        <section className="arcade-section">
          <div className="arcade-section-head">
            <h2 className="arcade-sh">From Kiwi's lab</h2>
            <span className="arcade-sh-meta">monetization stubs, kept for completeness</span>
          </div>
          <div className="arcade-grid">
            {labOpps.map((o) => (
      ...
      {playing && (
        <div className="arcade-overlay" onClick={() => setPlaying(null)}>
          <div className="arcade-stage" role="dialog" aria-modal="true" aria-label={playing.title} onClick={(e) => e.stopPropagation()}>
            <div className="arcade-stage-bar">
              <span className="arcade-stage-title">{playing.title}</span>
              <div className="arcade-stage-actions">
                <button type="button" className="btn btn-sm btn-secondary-sm arcade-restart" onClick={() => setPlaying({ ...playing })} title="Restart" aria-label="Restart"><i className="fa-solid fa-rotate-right" aria-hidden="true" /></button>
                <button type="button" className="btn btn-sm btn-secondary-sm" onClick={() => setPlaying(null)}><i className="fa-solid fa-xmark" aria-hidden="true" /> Done</button>
              </div>
            </div>
