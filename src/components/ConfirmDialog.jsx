...
export default function ConfirmDialog({ message, title = "Are you sure?", confirmLabel = "Delete", onConfirm, onCancel }) {
  ...
  return (
    ...
    >
      <div style={{ background: "var(--bg-card)", border: "1px solid var(--border-primary)", borderRadius: "0.75rem", padding: "1.5rem", maxWidth: "380px", width: "100%" }}>
        <h3 style={{ margin: "0 0 0.5rem", fontSize: "1rem", fontWeight: 600 }}>{title}</h3>
        {message && <p style={{ margin: "0 0 1.25rem", fontSize: "0.875rem", color: "var(--text-muted)", lineHeight: 1.5 }}>{message}</p>}
        <div style={{ display: "flex", gap: "0.5rem", justifyContent: "flex-end" }}>
          <button ref={cancelRef} className="btn" onClick={onCancel} style={{ background: "var(--bg-raised)", color: "var(--text-secondary)" }}>
            Cancel
          </button>
          <button className="btn" onClick={onConfirm} style={{ background: "var(--danger, var(--red))", color: "var(--text-on-accent)", border: "none" }}>
            {confirmLabel}
          </button>
        </div>
      </div>
