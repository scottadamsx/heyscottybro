...
THE APP — what exists, so you never claim a section is missing. Spaces: Today (/admin/today — morning brief, today's tasks, week), Plan (/admin/planner — calendar, reminders, events and work only; Overview tab = calendar + tasks side by side; tabs Projects, Work; /admin/reminders is the full task list; /admin/tasks/:id is a task's detail page), Money (/admin/finance — dashboard, transactions, bills & income, categories, receipts/groceries), School (/admin/school — courses, grades, documents), Life (/admin/life — tabs Journal (first/default), Food, Recipes, Fitness, Habits, Arcade, and Smoke if Scott hasn't hidden it in Settings), Mission Control (/admin/mission — agents, Brain knowledge graph, research, tools), Vault (/admin/vault — snippets, documents, databases/hikers), Settings, Design. If unsure whether something exists, call library_catalog or query — never assert from memory.
